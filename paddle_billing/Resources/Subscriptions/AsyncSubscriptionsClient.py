...
from paddle_billing.Entities.Collections import AsyncPaginator, SubscriptionCollection
...
class AsyncSubscriptionsClient:
    ...
    async def list(self, operation: ListSubscriptions | None = None) -> SubscriptionCollection:
        ...
        self.response = await self.client.get_raw("/subscriptions", operation.get_parameters())
        parser = ResponseParser(self.response)

        return SubscriptionCollection.from_list(
            parser.get_list(), AsyncPaginator(self.client, parser.get_pagination(), SubscriptionCollection)
        )
    ...
    async def get(self, subscription_id: str, includes=None) -> Subscription:
        ...
        self.response = await self.client.get_raw(f"/subscriptions/{subscription_id}", params)
        parser = ResponseParser(self.response)
    ...
    async def update(self, subscription_id: str, operation: UpdateSubscription) -> Subscription:
        self.response = await self.client.patch_raw(f"/subscriptions/{subscription_id}", operation)
        parser = ResponseParser(self.response)

        return Subscription.from_dict(parser.get_dict())

    async def pause(self, subscription_id: str, operation: PauseSubscription) -> Subscription:
        self.response = await self.client.post_raw(f"/subscriptions/{subscription_id}/pause", operation)
        parser = ResponseParser(self.response)

        return Subscription.from_dict(parser.get_dict())

    async def resume(self, subscription_id: str, operation: ResumeSubscription) -> Subscription:
        self.response = await self.client.post_raw(f"/subscriptions/{subscription_id}/resume", operation)
        parser = ResponseParser(self.response)

        return Subscription.from_dict(parser.get_dict())

    async def cancel(self, subscription_id: str, operation: CancelSubscription) -> Subscription:
        self.response = await self.client.post_raw(f"/subscriptions/{subscription_id}/cancel", operation)
        parser = ResponseParser(self.response)
    ...
    async def activate(self, subscription_id: str) -> Subscription:
        self.response = await self.client.post_raw(f"/subscriptions/{subscription_id}/activate")
        parser = ResponseParser(self.response)

        return Subscription.from_dict(parser.get_dict())

    async def create_one_time_charge(self, subscription_id: str, operation: CreateOneTimeCharge) -> Subscription:
        self.response = await self.client.post_raw(f"/subscriptions/{subscription_id}/charge", operation)
        parser = ResponseParser(self.response)

        return Subscription.from_dict(parser.get_dict())

    async def preview_update(
        self, subscription_id: str, operation: PreviewUpdateSubscription
    ) -> SubscriptionPreview:
        self.response = await self.client.patch_raw(f"/subscriptions/{subscription_id}/preview", operation)
        parser = ResponseParser(self.response)

        return SubscriptionPreview.from_dict(parser.get_dict())

    async def preview_one_time_charge(
        self, subscription_id: str, operation: PreviewOneTimeCharge
    ) -> SubscriptionPreview:
        self.response = await self.client.post_raw(f"/subscriptions/{subscription_id}/charge/preview", operation)
        parser = ResponseParser(self.response)
