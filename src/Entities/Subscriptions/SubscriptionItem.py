...
class SubscriptionItem:
    ...
    def from_dict(data: dict) -> SubscriptionItem:
        return SubscriptionItem(
            ...
            previously_billed_at = datetime.fromisoformat(data['previously_billed_at'])  if 'previously_billed_at' in data else None,
            next_billed_at       = datetime.fromisoformat(data['next_billed_at'])        if 'next_billed_at'       in data else None,
            trial_dates          = SubscriptionTimePeriod.from_dict(data['trial_dates']) if 'trial_dates'          in data else None,
