...
class TestSimulationsClient:
    ...
    def test_list_simulation_runs_can_paginate(
        ...
        for simulation_run in response:
            allSimulationRuns.append(simulation_run)

        assert len(allSimulationRuns) == 2
    ...
    def test_get_simulation_run_returns_expected_response(
        ...
        simulation = test_client.client.simulation_runs.get(simulation_id, simulation_run_id)
        response_json = test_client.client.simulation_runs.response.json()
        last_request = mock_requests.last_request
        ...
        assert simulation.id == simulation_run_id
        assert simulation.status == SimulationRunStatus.Pending
        assert simulation.type == SimulationScenarioType.SubscriptionCreation
        assert simulation.created_at.isoformat() == "2024-09-18T12:17:04.168467+00:00"
        assert simulation.updated_at.isoformat() == "2024-09-18T12:17:04.168467+00:00"
