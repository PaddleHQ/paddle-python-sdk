...
from paddle_billing.Resources.Discounts.Operations import CreateDiscount, DiscountInclude, ListDiscounts, UpdateDiscount
...
class TestDiscountsClient:
    ...
    def test_create_discount_response_has_custom_data(
        ...
        mock_requests.post(
            f"{test_client.base_url}/discounts",
            status_code=200,
    ...
    def test_update_discount_response_has_custom_data(
        ...
        response = test_client.client.discounts.update(
            "dsc_01h83xenpcfjyhkqr4x214m02x", UpdateDiscount(enabled_for_checkout=False)
        )
    ...
    @mark.parametrize(
        ...
        [
            ...
            (
                ListDiscounts(includes=[DiscountInclude.DiscountGroup]),
                200,
                ReadsFixtures.read_raw_json_fixture("response/list_default"),
                "/discounts?include=discount_group",
            ),
            (
                ListDiscounts(mode=DiscountMode.Standard),
                200,
                ReadsFixtures.read_raw_json_fixture("response/list_default"),
                "/discounts?mode=standard",
            ),
            (
                ListDiscounts(mode=DiscountMode.Custom),
                200,
                ReadsFixtures.read_raw_json_fixture("response/list_default"),
                "/discounts?mode=custom",
            ),
        ],
        ids=[
            ...
            "List discounts with discount group included",
            "List discounts filtered by standard mode",
            "List discounts filtered by custom mode",
        ],
    )
    ...
    @mark.parametrize(
        ...
        [
            (
                "dsc_01h83xenpcfjyhkqr4x214m02x",
                200,
                ReadsFixtures.read_raw_json_fixture("response/full_entity_include_discount_group"),
                "/discounts/dsc_01h83xenpcfjyhkqr4x214m02x?include=discount_group",
            )
        ],
        ids=["Get discount with discount group include"],
    )
    def test_get_discounts_returns_expected_response_with_discount_group(
        self,
        test_client,
        mock_requests,
        discount_id,
        ...
        response = test_client.client.discounts.get(discount_id, GetDiscount(includes=[DiscountInclude.DiscountGroup]))
        response_json = test_client.client.discounts.response.json()
        last_request = mock_requests.last_request
