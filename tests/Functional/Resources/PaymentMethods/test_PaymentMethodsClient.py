...
class TestPaymentMethodsClient:
    ...
    def test_get_payment_methods_returns_expected_south_korea_local_card_response(
        self,
        test_client,
        mock_requests,
    ):
        ...
        payment_method_id = "paymtd_01hs8zx6x377xfsfrt2bqsevbw"
        expected_response_status = 200
        expected_response_body = ReadsFixtures.read_raw_json_fixture("response/full_entity_south_korea_local_card")
        expected_path = "/customers/ctm_01hv6y1jedq4p1n0yqn5ba3ky4/payment-methods/paymtd_01hs8zx6x377xfsfrt2bqsevbw"
        ...
        assert response.card is None
        assert response.paypal is None
        assert response.type == SavedPaymentMethodType.SouthKoreaLocalCard
        assert response.origin == SavedPaymentMethodOrigin.Subscription
        assert response.saved_at.isoformat() == "2024-05-03T11:50:23.422000+00:00"
        assert response.updated_at.isoformat() == "2024-05-04T11:50:23.422000+00:00"

        assert response.south_korea_local_card is not None
        assert response.south_korea_local_card.type == SouthKoreaLocalCardType.Kookmin
        assert response.south_korea_local_card.last4 == "4242"
